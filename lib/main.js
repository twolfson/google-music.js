...
proto.queue = {
  ...
  _render: function (container, force) {
    ...
    if (container.style.display === 'none' && (!table || force)) {
      ...
      // Render queue
      this.queue._fireEvent(document.querySelector(SELECTORS.queue.triggerButton), 'click');
      setTimeout(function () {
  ...
  playSong: function (id) {
    var songRow = this.doc.querySelector('[data-id="' + id + '"]');
    assert(songRow, 'Failed to find song with ID: ' + id);
